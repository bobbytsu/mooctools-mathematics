...
<head>
    ...
    <title>Kindergarten - Addition</title>
    ...
    <div class="container my-5 bg-light">
        <div class="p-5">
            ...
            <ul class="nav nav-tabs border bg-white">
                <li class="nav-item">
                    <a class="nav-link{{ Request::is('kindergarten/addition') ? ' active' : '' }} border-top-0 border-left-0 border-right-0 text-dark" href="{{ route('addition-kindergarten') }}">Addition</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ Request::is('kindergarten/subtraction') ? ' active' : '' }} border-top-0 border-left-0 border-right-0 text-dark" href="{{ route('subtraction-kindergarten') }}">Subtraction</a>
                </li>
            ...
            <form class="form-inline" method="POST" action="{{ route('additionkindergartensolution') }}">
                @csrf
            ...
            <div class="bg-white border p-4">
                <form method="POST" action="{{ route('additionkindergartensolution') }}">
                    @csrf
                ...
                @php
                    ...
                    $delay = 1;
                @endphp
                <p class="drop-in" style="animation-delay: {{$delay}}s" >Imagine you have <b>{{$input_1}}</b> cand{{ $input_1 > 1 ? 'ies' : 'y' }}.</p>
                @for($i=1; $i <= $input_1; $i++)
                    @php
                        $delay += $animationDelay;
                    @endphp
                    <img src="{{ asset('assets/candy-cane.png') }}" class="my-2 candy drop-in" height="100px" alt="candy {{$i}}" style="animation-delay: {{$delay}}s">
                @endfor
                @php
                    $delay += 1;
                @endphp
                <p class="mt-2 drop-in" style="animation-delay: {{$delay}}s">Then you get another <b>{{$input_2}}</b> cand{{ $input_2 > 1 ? 'ies' : 'y' }}.</p>
                @for($i=1; $i <= $input_2; $i++)
                    @php
                        $delay += $animationDelay;
                    @endphp
                    <img src="{{ asset('assets/candy-cane.png') }}" class="my-2 drop-in" height="100px" alt="candy {{$i}}" style="animation-delay: {{$delay}}s">
                @endfor
                @php
                    $delay += 1;
                @endphp
                <p class="mt-2 drop-in" style="animation-delay: {{$delay}}s">Therefore you will get <b>{{$result}}</b> cand{{ $result > 1 ? 'ies' : 'y' }} in total.</p>
                @for($i=1; $i <= $result; $i++)
                    @php
                        $delay += $animationDelay;
                    @endphp
                ...
                @php
                    $delay += 1;
                @endphp
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay}}s">Result</h5>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 1}}s"><b>{{$result}}</b></h5>
            </div>
            <!-- Solution -->
        </div>
    </div>
