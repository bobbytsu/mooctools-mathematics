...
<head>
    ...
    <title>Kindergarten - Subtraction</title>
...
<body>
    ...
    <div class="container my-5 bg-light">
        <div class="p-5">
            ...
            <form class="form-inline" method="POST" action="{{ route('subtractionkindergartensolution') }}">
                @csrf
            ...
            <div class="bg-white border p-4">
                <form method="POST" action="{{ route('subtractionkindergartensolution') }}">
                    @csrf
                ...
                <p class="mt-2 drop-in" style="animation-delay: {{$delay + 2}}s">Then you give away <b>{{$input_2}}</b> cand{{ $input_2 > 1 ? 'ies' : 'y' }}.</p>
                <span class="drop-in" style="animation-delay: {{$delay + 1}}s">
                    ...
                    @for($i=1; $i <= $input_1; $i++)
                        ...
                        @endphp
                        <img src="{{ asset('assets/candy-cane.png') }}" class="my-2 candy @if($i > $result) drop-out @endif" height="100px" alt="candy {{$i}}" style="animation-delay: {{$delay}}s">
                    @endfor
                </span>
                @php
                    $delay += 1;
                @endphp
                <p class="mt-2 drop-in" style="animation-delay: {{$delay}}s">Therefore you will have <b>{{$result}}</b> cand{{ $result > 1 ? 'ies' : 'y' }} remaining.</p>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 1}}s">Result</h5>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 2}}s"><b>{{$result}}</b></h5>
            </div>
            <!-- Solution -->
        </div>
    </div>
