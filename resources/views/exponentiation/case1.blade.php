...
<head>
    ...
    <title>Elementary - Exponentiation</title>
...
<body>
    ...
    <div class="container my-5 bg-light">
        <div class="p-5">
            ...
            <form class="form-inline" method="POST" action="{{ route('exponentiationsolution') }}">
                @csrf
                <div class="form-group">
                    <input class="form-control text-center rounded-0 my-2 mr-4 square-input" name="input_1" value="{{ $input_1 }}">
                    <h4 class="my-2 mr-4 d-xl-none d-lg-none d-md-none">pow</h4>
                    <input class="form-control text-center rounded-0 p-0 mt-2 mb-5 mr-4 square-root-input" name="input_2" value="{{ $input_2 }}">
            ...
            <div class="bg-white border p-4">
                <form method="POST" action="{{ route('exponentiationsolution') }}">
                    @csrf
                ...
                <div class="m-5">
                    <small class="drop-in exponentiation-input2" style="animation-delay: {{$delay + 1}}s">
                        {{$input_2}}
                    </small>
                    <small class="drop-in exponentiation-1minus1" style="animation-delay: {{$delay  + 1}}s">
                        1 - 1
                    </small>
                    <h5 class="drop-in" style="animation-delay: {{$delay  + 1}}s">
                        <span>{{$input_1}}</span>
                        <span class="ml-3">=</span>
                        <span class="ml-3">{{$input_1}}</span>
                    </h5>
                    <small class="drop-in exponentiation-pow1" style="animation-delay: {{$delay  + 2}}s">
                        1
                    </small>
                    <small class="drop-in exponentiation-powminus1" style="animation-delay: {{$delay + 2}}s">
                        -1
                    </small>
                    <h5 class="drop-in ml-2" style="animation-delay: {{$delay + 2}}s">
                        <span class="ml-4">=</span>
                        <span class="ml-3">{{$input_1}}</span>
                        <span class="ml-3">.</span>
                        <span class="ml-3">{{$input_1}}</span>
                    </h5>
                    <small class="drop-in exponentiation-pow1" style="animation-delay: {{$delay + 3}}s">
                        1
                    </small>
                    <h5 class="drop-in ml-2" style="animation-delay: {{$delay + 3}}s">
                        <span class="ml-4">=</span>
                        <span class="ml-4">{{$input_1}}</span>      
                    </h5>
                    <hr class="drop-in hr-2" style="animation-delay: {{$delay + 3}}s">
                    <small class="drop-in exponentiation-pow1" style="animation-delay: {{$delay + 3}}s">
                        1
                    </small>
                    <h5 class="drop-in ml-5" style="animation-delay: {{$delay + 3}}s">
                        <span class="ml-4">{{$input_1}}</span>      
                    </h5>
                    <h5 class="drop-in ml-2 mt-4" style="animation-delay: {{$delay + 4}}s">
                        <span class="ml-4">=</span>    
                        <span class="ml-4">{{$result}}</span>    
                    </h5>
                </div>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 5}}s">Result</h5>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 6}}s"><b>{{$result}}</b></h5>
            </div>
            <!-- Solution -->
        </div>
    </div>
