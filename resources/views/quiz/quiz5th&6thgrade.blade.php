...
<head>
    ...
    <title>Quiz 5th & 6th Grade</title>
...
<body>
    ...
    <div class="container my-5 bg-light">
        <div class="p-5">
            ...
            <h4 class="text-center my-5">Quiz: 5<sup>th</sup> & 6<sup>th</sup> Grade</h4>
            <h5 class="mb-5">Question</h5>
            <form method="POST" action="{{ route('quiz5th6thgraderesult') }}" onsubmit="return confirm('Are you sure want to submit these answers?')">
                @csrf
                @php
                    $i = 1;
                @endphp
                @foreach($questions->shuffle()->take(10) as $question)
                    ...
                    <div class="form-group row d-flex align-items-center">
                        <label for="question{{ $i }}" class="col-auto col-form-label">
                            <h5>{{ $i }}.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{!! $formattedQuestion !!}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=</h5>
                        </label>
                        <div class="col-auto">
                            <input type="text" class="form-control text-center rounded-0 p-0 square-input" id="question{{ $i }}" name="question{{ $i }}" value="{{ old('question'.$i) }}">                           
                        </div>
                        @error('question'.$i)
