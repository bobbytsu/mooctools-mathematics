...
<head>
    ...
    <title>Elementary - Addition</title>
...
<body>
    ...
    <div class="container my-5 bg-light">
        <div class="p-5">
            ...
            <form class="form-inline" method="POST" action="{{ route('additionsolution') }}">
                @csrf
            ...
            <div class="bg-white border p-4">
                <form method="POST" action="{{ route('additionsolution') }}">
                    @csrf
                ...
                <p class="drop-in" style="animation-delay: {{$delay}}s"><b>{{$input_1}} + {{$input_2}} = {{$input_2}} + {{$input_1}}</b></p>
                <p class="drop-in" style="animation-delay: {{$delay + 3}}s">This is called <b>Commutative Property</b></p>
                <p class="drop-in" style="animation-delay: {{$delay + 7}}s">It means that by changing the order of the numbers does not change the result of the sum</p>
                ...
                <div class="m-5">
                    <h5 class="ml-5 drop-in" style="animation-delay: {{$delay + 13}}s">
                        <span class="color-animation" style="animation-delay: {{$delay + 19}}s">{{$case_3_input_2_digit_1}}</span>
                        <span class="color-animation" style="animation-delay: {{$delay + 14}}s">{{$case_3_input_2_digit_2}}</span>
                    </h5>
                    <h5 class="ml-case-2-a-b-input-2 drop-in" style="animation-delay: {{$delay + 13}}s">
                        <span class="color-animation" style="animation-delay: {{$delay + 16}}s">{{$case_3_input_1_digit_1}}</span>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <span class="color-animation" style="animation-delay: {{$delay + 15}}s">+</span>
                    </h5>
                    <hr class="drop-in" style="animation-delay: {{$delay + 13}}s">
                    <h5 class="mt-2 ml-5">
                        <span class="drop-in" style="animation-delay: {{$delay + 21}}s">{{$result_digit_1}}</span>
                        <span class="drop-in" style="animation-delay: {{$delay + 18}}s">{{$result_digit_2}}</span>
                    </h5>
                </div>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 22}}s">Result</h5>
                <h5 class="mt-2 drop-in" style="animation-delay: {{$delay + 23}}s"><b>{{$result}}</b></h5>
            </div>
            <!-- Solution -->
        </div>
    </div>
